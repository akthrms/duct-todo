(ns duct-todo.handler.articles-test
...
(t/deftest handler-articles-test
  (t/testing "GET /articles"
    (let [handler (ig/init-key :duct-todo.handler.articles/index {:db database-stub})
          [status message] (handler {})]
      (t/is (= :ataraxy.response/ok status))
      (t/is (= [{:id 1 :content "foo"}] message))))

  (t/testing "POST /articles"
    (let [handler (ig/init-key :duct-todo.handler.articles/create {:db database-stub})
          [status message] (handler {})]
